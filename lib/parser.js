...
function buildOrderCardPayload(card, errors) {
  ...
  const labelsList = (card.orderLabels || []).map(normalizeFieldValue).filter(Boolean);
  const answerRaw = normalizeFieldValue(card.orderAnswerRaw || "");
  const line = card.typeLine || 1;

  if (!front && !tokensList.length) return null;
  if (!front) {
    errors.push({ line, message: "ORDER requiere FRONT." });
    return null;
  }
  if (!tokensList.length || !labelsList.length) {
    errors.push({ line, message: "ORDER requiere TOKENS y LABELS." });
    return null;
  }
  if (tokensList.length !== labelsList.length) {
  ...
  if (!answerRaw) {
    errors.push({ line, message: "ORDER requiere ANSWER." });
    return null;
  }
