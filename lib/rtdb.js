...
function buildFolderPayload(folderId, folder, now) {
  ...
  let name = folder?.name || nameFromPath || "";
  if (!path && name) {
    path = normalizeFolderPath(name);
  }
  if (!name) {
    name = path || folderId || "Carpeta";
  }
  const payload = {
    ...folder,
    id: folderId,
    name,
    parentId: typeof folder?.parentId === "undefined" ? null : folder.parentId,
    path: path || name,
    source: folder?.source === "import" ? "import" : "manual",
  };
  if (!folder?.createdAt && now) {
    payload.createdAt = now;
  }
  if (!folder?.updatedAt && now) {
    payload.updatedAt = folder?.createdAt || now;
  }
  return payload;
}
...
export async function normalizeFoldersSnapshot(db, username, folders) {
  ...
  Object.entries(folders || {}).forEach(([key, folder]) => {
    const mismatchId = folder?.id && folder.id !== key;
    const folderId = mismatchId ? key : (folder?.id || key);
    const payload = buildFolderPayload(folderId, folder, now);
    normalized[folderId] = payload;
    if (!folder?.id || mismatchId) {
      updates[`${root}/folders/${key}/id`] = folderId;
    }
    if (!folder?.name || folder.name !== payload.name) {
      updates[`${root}/folders/${key}/name`] = payload.name;
    }
    if (!folder?.path || normalizeFolderPath(folder.path) !== payload.path) {
      updates[`${root}/folders/${key}/path`] = payload.path;
    }
    if (typeof folder?.parentId === "undefined") {
      updates[`${root}/folders/${key}/parentId`] = payload.parentId;
    }
    if (!folder?.createdAt) {
      updates[`${root}/folders/${key}/createdAt`] = payload.createdAt;
    }
    if (!folder?.source || (folder.source !== "manual" && folder.source !== "import")) {
      updates[`${root}/folders/${key}/source`] = payload.source;
    }
    if (!folder?.updatedAt) {
      updates[`${root}/folders/${key}/updatedAt`] = payload.updatedAt;
    }
  });
...
export function listenManualFolders(db, username, callback) {
  const root = userRoot(username);
  return onValue(ref(db, `${root}/manualFolders`), (snap) => {
    callback(snap.exists() ? snap.val() : {});
  });
}
...
export async function createManualFolder(db, username, folder) {
  const root = userRoot(username);
  const now = Date.now();
  const id = folder?.id || (typeof crypto !== "undefined" && crypto.randomUUID
    ? crypto.randomUUID()
    : `folder_${Date.now()}_${Math.random().toString(16).slice(2, 8)}`);
  const name = String(folder?.name || "Carpeta").trim() || "Carpeta";
  const payload = {
    id,
    name,
    path: name,
  ...
  await set(ref(db, `${root}/manualFolders/${id}`), payload);
  return id;
}
...
export async function getOrCreateFolderByPath(db, username, path, cachedFolders = null) {
  ...
  const payload = buildFolderPayload(id, { name: segments[segments.length - 1], path: normalizedPath }, now);
  payload.createdAt = now;
  payload.updatedAt = now;
  await set(ref(db, `${root}/folders/${id}`), payload);
  if (foldersMap) {
...
export async function deleteManualFolder(db, username, folderId) {
  const root = userRoot(username);
  const updates = {};
  updates[`${root}/manualFolders/${folderId}`] = null;
  updates[`${root}/manualCardsByFolder/${folderId}`] = null;
  await update(ref(db), updates);
}
...
export async function fetchManualCardsByFolder(db, username, folderId, limit = 500) {
  if (!folderId) return [];
  const root = userRoot(username);
  const cardsRef = ref(db, `${root}/manualCardsByFolder/${folderId}`);
...
export async function upsertManualCard(db, username, folderId, card) {
  if (!folderId || !card?.id) return null;
  const root = userRoot(username);
  const now = Date.now();
  const payload = {
    ...card,
  ...
  await update(ref(db, `${root}/manualCardsByFolder/${folderId}/${card.id}`), payload);
  return payload;
}
