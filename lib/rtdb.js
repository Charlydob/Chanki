...
export async function ensureVocabFolders(db, username, existingFolders = {}) {
  ...
  let folderList = Object.values(existingFolders || {});
  if (!folderList.length) {
    const root = userRoot(username);
    const snap = await get(ref(db, `${root}/folders`));
    folderList = snap.exists() ? Object.values(snap.val()) : [];
  }
  const findByPath = (path) =>
    folderList.find((folder) => folder.path === path || folder.name === path);
  const result = {};
  for (const [key, path] of Object.entries(requiredPaths)) {
    ...
    if (found) {
      ...
      result[key] = await createFolder(db, username, { name: path });
    }
  }
  return result;
}
