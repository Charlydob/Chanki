...
import { buildQueueKey, defaultSrs, normalizeSrs } from "./srs.js";
...
function folderNameFromLegacyPath(path) {
...
function buildFolderPayload(folderId, folder, now) {
  const legacyName = folderNameFromLegacyPath(folder?.path || "");
  const name = String(folder?.name || legacyName || "Carpeta").trim() || "Carpeta";
  return {
    id: folderId,
    name,
    emoji: String(folder?.emoji || "📁").trim() || "📁",
    color: String(folder?.color || "#8b5cf6").trim() || "#8b5cf6",
    sourceLang: String(folder?.sourceLang || "es").trim().toLowerCase() || "es",
    targetLang: String(folder?.targetLang || "de").trim().toLowerCase() || "de",
...
export async function normalizeFoldersSnapshot(db, username, folders) {
  const root = userRoot(username);
  const normalized = {};
  const updates = {};
  const now = Date.now();
  Object.entries(folders || {}).forEach(([key, folder]) => {
    const folderId = folder?.id || key;
    const payload = buildFolderPayload(folderId, folder, now);
    normalized[folderId] = payload;
    const base = `${root}/folders/${key}`;
    if (folderId !== key || folder?.id !== folderId) updates[`${base}/id`] = folderId;
    if (folder?.name !== payload.name) updates[`${base}/name`] = payload.name;
    if (folder?.emoji !== payload.emoji) updates[`${base}/emoji`] = payload.emoji;
    if (folder?.color !== payload.color) updates[`${base}/color`] = payload.color;
    if (folder?.sourceLang !== payload.sourceLang) updates[`${base}/sourceLang`] = payload.sourceLang;
    if (folder?.targetLang !== payload.targetLang) updates[`${base}/targetLang`] = payload.targetLang;
    if (!folder?.createdAt) updates[`${base}/createdAt`] = payload.createdAt;
    if (!folder?.updatedAt) updates[`${base}/updatedAt`] = payload.updatedAt;
    if (typeof folder?.path !== "undefined") updates[`${base}/path`] = null;
    if (folder?.parentId !== payload.parentId) updates[`${base}/parentId`] = payload.parentId;
    if (typeof folder?.source !== "undefined") updates[`${base}/source`] = null;
  });
  if (Object.keys(updates).length) await update(ref(db), updates);
  return { folders: normalized, repaired: Object.keys(updates).length };
}
...
function areCardContentsEqual(existing, incoming) {
  ...
  if (existingType === "order") {
    const existingTokens = existing.orderTokens || [];
    const incomingTokens = incoming.orderTokens || [];
    if (existingTokens.length !== incomingTokens.length) return false;
    for (let i = 0; i < existingTokens.length; i += 1) {
      const existingToken = existingTokens[i] || {};
      const incomingToken = incomingTokens[i] || {};
      if (normalizeContentValue(existingToken.id) !== normalizeContentValue(incomingToken.id)) return false;
      if (normalizeContentValue(existingToken.text) !== normalizeContentValue(incomingToken.text)) return false;
      if (normalizeContentValue(existingToken.label) !== normalizeContentValue(incomingToken.label)) return false;
    }
    const existingAnswer = existing.orderAnswer || [];
    const incomingAnswer = incoming.orderAnswer || [];
    if (existingAnswer.length !== incomingAnswer.length) return false;
    for (let i = 0; i < existingAnswer.length; i += 1) {
      if (normalizeContentValue(existingAnswer[i]) !== normalizeContentValue(incomingAnswer[i])) return false;
    }
    return normalizeContentValue(existing.front || "") === normalizeContentValue(incoming.front || "");
  }
  return (
...
export function usersPublicRoot() {
  return "/usersPublic";
}

export async function upsertUserPublic(db, uid, data) {
  if (!uid) return;
  await update(ref(db, `${usersPublicRoot()}/${uid}`), {
    ...data,
    updatedAt: Date.now(),
  });
}

export async function fetchUsersPublic(db) {
  const snap = await get(ref(db, usersPublicRoot()));
  return snap.exists() ? snap.val() : {};
}

export function listenSharedFoldersByUser(db, uid, callback) {
  return onValue(ref(db, `/sharedFoldersByUser/${uid}`), (snap) => {
    callback(snap.exists() ? snap.val() : {});
  });
}

export function listenFolderShares(db, ownerUid, folderId, callback) {
  return onValue(ref(db, `/folderShares/${ownerUid}/${folderId}`), (snap) => {
    callback(snap.exists() ? snap.val() : {});
  });
}

export async function shareFolder(db, { ownerUid, folderId, sharedUid, role, addedBy }) {
  if (!ownerUid || !folderId || !sharedUid) return;
  const now = Date.now();
  const shareKey = `${ownerUid}_${folderId}`;
  const payload = {
    role: role || "viewer",
    addedAt: now,
    addedBy: addedBy || ownerUid,
  };
  const updates = {};
  updates[`/folderShares/${ownerUid}/${folderId}/${sharedUid}`] = payload;
  updates[`/sharedFoldersByUser/${sharedUid}/${shareKey}`] = {
    ownerUid,
    folderId,
    role: payload.role,
    addedAt: now,
...
export async function unshareFolder(db, { ownerUid, folderId, sharedUid }) {
  if (!ownerUid || !folderId || !sharedUid) return;
  const shareKey = `${ownerUid}_${folderId}`;
  const updates = {};
  updates[`/folderShares/${ownerUid}/${folderId}/${sharedUid}`] = null;
  updates[`/sharedFoldersByUser/${sharedUid}/${shareKey}`] = null;
  await update(ref(db), updates);
}
...
export function listenFolderById(db, username, folderId, callback) {
  const root = userRoot(username);
  return onValue(ref(db, `${root}/folders/${folderId}`), (snap) => {
    callback(snap.exists() ? snap.val() : null);
  });
}

export function listenCardsByUser(db, username, callback) {
  const root = userRoot(username);
  return onValue(ref(db, `${root}/cards`), (snap) => {
    callback(snap.exists() ? snap.val() : {});
  });
}
...
export async function createFolder(db, username, folder) {
  ...
  const payload = buildFolderPayload(id, folder, now);
...
export async function getOrCreateFolderByPath(db, username, path, cachedFolders = null) {
  ...
  const legacyName = folderNameFromLegacyPath(normalizedPath);
  let foldersMap = cachedFolders;
  if (!foldersMap) {
    const snap = await get(ref(db, `${root}/folders`));
    foldersMap = snap.exists() ? snap.val() : {};
  }
  const found = Object.values(foldersMap || {}).find((folder) => String(folder?.name || "").trim().toLowerCase() === legacyName.toLowerCase());
  if (found) return found.id;
  const now = Date.now();
  const id = typeof crypto !== "undefined" && crypto.randomUUID
    ? crypto.randomUUID()
    : `folder_${Date.now()}_${Math.random().toString(16).slice(2, 8)}`;
  const payload = buildFolderPayload(id, { name: legacyName || "Importadas" }, now);
  await set(ref(db, `${root}/folders/${id}`), payload);
  if (foldersMap) foldersMap[id] = payload;
  return id;
}
...
export async function deleteFolder(db, username, folderId) {
  const root = userRoot(username);
  const cards = await fetchCardsByFolderId(db, username, folderId, 5000);
  const updates = { [`${root}/folders/${folderId}`]: null };
  cards.forEach((card) => {
    updates[`${root}/cards/${card.id}/folderId`] = null;
  });
  await update(ref(db), updates);
}
...
export async function createCard(db, username, card, options = null) {
  ...
  const payload = {
    ...
    orderLabelsCatalog: card.orderLabelsCatalog || [],
    orderTokenLabels: card.orderTokenLabels || {},
    labels: card.labels || "",
    tags: card.tags || {},
    sourceLang: card.sourceLang || "",
    targetLang: card.targetLang || "",
    cardGrammarType: card.cardGrammarType || "normal",
    nounGender: card.nounGender || null,
    conjugationBlocks: card.conjugationBlocks || [],
...
export async function updateCard(db, username, cardId, updates, options = {}) {
  ...
  if (!existing.srs || !existing.srs.bucket || !existing.srs.dueAt) {
    queueUpdates[`${root}/cards/${cardId}/srs`] = normalizeSrs({
      ...existing.srs,
      bucket,
      dueAt,
      nextReviewAt: existing.srs?.nextReviewAt || dueAt,
...
export async function updateReview(db, username, card, nextSrs) {
  const root = userRoot(username);
  const currentSrs = normalizeSrs(card.srs, card.createdAt);
  const normalizedNext = normalizeSrs(nextSrs, card.createdAt);
  const oldKey = buildQueueKey(currentSrs.dueAt, card.id);
  const newKey = buildQueueKey(normalizedNext.dueAt, card.id);
  const updates = {};
  updates[`${root}/cards/${card.id}/srs`] = normalizedNext;
  updates[`${root}/cards/${card.id}/updatedAt`] = Date.now();
  updates[`${root}/queue/${currentSrs.bucket}/${oldKey}`] = null;
  updates[`${root}/queue/${normalizedNext.bucket}/${newKey}`] = true;
  if (card.folderId) {
    updates[`${root}/folderQueue/${card.folderId}/${currentSrs.bucket}/${oldKey}`] = null;
    updates[`${root}/folderQueue/${card.folderId}/${normalizedNext.bucket}/${newKey}`] = true;
  }
  console.info("[chanki:review:update-card]", {
    cardId: card.id,
    fromBucket: currentSrs.bucket,
    toBucket: normalizedNext.bucket,
    reviewCount: normalizedNext.reviewCount,
    mistakes: normalizedNext.mistakes,
    correctCount: normalizedNext.correctCount,
    lastReviewedAt: normalizedNext.lastReviewedAt,
    nextReviewAt: normalizedNext.nextReviewAt,
    intervalMinutes: normalizedNext.intervalMinutes,
    intervalDays: normalizedNext.intervalDays,
    ease: normalizedNext.ease,
    srsLevel: normalizedNext.srsLevel,
    averageGrade: normalizedNext.averageGrade,
    reviewScore: normalizedNext.reviewScore,
  });
  if (currentSrs.bucket !== normalizedNext.bucket) {
    updates[`${root}/stats/bucketCounts/${currentSrs.bucket}`] = increment(-1);
    updates[`${root}/stats/bucketCounts/${normalizedNext.bucket}`] = increment(1);
    if (currentSrs.bucket === "new" && normalizedNext.bucket !== "new") {
      updates[`${root}/stats/totals/newCards`] = increment(-1);
      updates[`${root}/stats/totals/learnedCards`] = increment(1);
    }
  }
...
export async function fetchCardsByFolderPathCompat(db, username, folderPath, limit = 500) {
  const normalizedPath = normalizeFolderPath(folderPath || "");
  if (!normalizedPath) return [];
  const root = userRoot(username);
  const cardsRef = ref(db, `${root}/cards`);
  const cardsQuery = query(cardsRef, orderByChild("createdAt"), limitToFirst(Math.max(limit * 5, 500)));
  const snap = await get(cardsQuery);
  const entries = [];
  if (snap.exists()) {
    snap.forEach((childSnap) => {
      ...
      const path = normalizeFolderPath(value.folderPath || "");
      if (path === normalizedPath) {
        entries.push({ id: childSnap.key, ...value });
      }
    });
  }
...
export async function buildSessionQueue({
    ...
    for (const bucket of buckets) {
      ...
      for (let i = 0; i < cardIdsForBucket.length; i += chunkSize) {
        ...
        snaps.forEach((snap) => {
          ...
          const includeMatch = !normalizedTags.length || (tagFilterMode === "and"
            ? normalizedTags.every((tag) => cardTags.includes(tag))
            : normalizedTags.some((tag) => cardTags.includes(tag)));
          const excludeMatch = normalizedExcludeTags.length && normalizedExcludeTags.some((tag) => cardTags.includes(tag));
  ...
  const filtered = fallbackCards.filter((card) => {
    ...
    if (normalizedExcludeTags.length && normalizedExcludeTags.some((tag) => cardTags.includes(tag))) {
      return false;
    }
...
export async function repairIndexesOnce(db, username, cards = null, limit = 200) {
  ...
  cardsToRepair.slice(0, limit).forEach((card) => {
    ...
    if (!card.srs || !card.srs.bucket || !card.srs.dueAt) {
      updates[`${root}/cards/${card.id}/srs`] = normalizeSrs({
        ...card.srs,
        bucket,
        dueAt,
        nextReviewAt: card.srs?.nextReviewAt || dueAt,
...
export async function migrateCardsFolderIdsOnce(db, username, limit = 2000) {
  ...
  const resolveFolderId = async (legacyPath) => {
    const normalized = normalizeFolderPath(legacyPath || "");
    if (!normalized) return null;
    return getOrCreateFolderByPath(db, username, normalized, foldersMap);
  };
  ...
  cardsSnap.forEach((childSnap) => {
    ...
    const card = childSnap.val() || {};
    if (!cardId) return;
    if (card.folderId && folderIdSet.has(card.folderId) && !card.folderPath) return;
    tasks.push((async () => {
      let nextFolderId = card.folderId && folderIdSet.has(card.folderId) ? card.folderId : null;
      if (!nextFolderId) {
        nextFolderId = await resolveFolderId(card.folderPath || card.folderId);
      }
      if (nextFolderId) {
        updates[`${root}/cards/${cardId}/folderId`] = nextFolderId;
      } else {
        updates[`${root}/cards/${cardId}/folderId`] = null;
      }
      if (typeof card.folderPath !== "undefined") {
        updates[`${root}/cards/${cardId}/folderPath`] = null;
      }
      migrated += 1;
    })());
  });
...
export function listenLexicon(db, uid, callback) {
  if (!uid) return () => {};
  return onValue(ref(db, `/lexicon/${uid}`), (snap) => {
    callback(snap.exists() ? snap.val() : {});
  });
}

export async function upsertLexiconEntry(db, uid, termKey, meaning) {
  if (!uid || !termKey) return;
  await update(ref(db), {
    [`/lexicon/${uid}/${termKey}`]: {
      meaning,
      updatedAt: Date.now(),
