...
function buildManualFolderList() {
  return Object.entries(state.manualFolders || {}).map(([folderKey, folder]) => {
...
export function renderFolderSelects() {
  ...
  const manualFolders = buildManualFolderList().filter((folder) => !query || folder._search.includes(query));
  ...
  if (manualFolders.length) {
    ...
    title.textContent = "Mis carpetas manuales";
    options.appendChild(title);
    manualFolders.forEach((folder) => {
      addOption(folder.id, `${folder.name} · Manual`, selected.has(folder.id));
    });
  }
  ...
  if (!manualFolders.length && !ownedFolders.length && !sharedEntries.length && query) {
  ...
  if (elements.reviewFolderLabel) {
    if (!selected.size) {
      ...
      if (value.startsWith("shared:")) {
        ...
        elements.reviewFolderLabel.textContent = state.manualFolders?.[value]?.name || state.folders[value]?.name || "Carpeta";
      }
    } else {
...
export function renderFolders() {
  ...
  manualList.forEach((folder) => {
    ...
    const menuId = `manual-folder-menu-${folder.id}`;
    const subtitle = typeof folder.cardCount === "number"
      ? `${folder.cardCount} tarjetas · Manual`
      : "Manual";
    item.innerHTML = `
      <button class="item-main" data-action="select" data-id="${folder.id}" data-owner-uid="${state.username}" data-source="manual" type="button">
        ...
        <span class="item-text">
          <span class="item-title-row">
            ...
            <span class="share-badge">Manual</span>
          </span>
